...
public class TableFieldInfo implements Constants {
    ...
    public String getSqlSet(final boolean ignoreIf, final String prefix) {
        ...
        String fieldIfNotNull = convertIf(sqlSet, convertIfProperty(newPrefix, property), updateStrategy);
        if (null == fieldIfNotNull) {
            return null;
        }
        // 重写 mybatis plus 添加字段自定义动态update（接口传入key为null表示为清空，没传key表示不更新）
        String fieldSetNull = SqlScriptUtils.convertIf(sqlSet,
                String.format("%s == null and @com.mmyf.commons.util.mybatis.DynamicUpdateUtils@needForceUpdateToNull('%s')"
                        , convertIfProperty(newPrefix, property)
                        , property
                ),
                false);
        // 重写 mybatis plus 添加字段自定义动态update（接口传入key为null表示为清空，没传key表示不更新）
        return fieldIfNotNull + fieldSetNull;
    }
