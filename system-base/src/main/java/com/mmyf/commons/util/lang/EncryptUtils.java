...
public class EncryptUtils {
    private static final String PASSWORD_CRYPT_KEY = "teddy_crs";
    ...
    public final static String decrypt(String data) {
        try {
            return new String(decrypt(hex2byte(data.getBytes("UTF-8")),
                    PASSWORD_CRYPT_KEY.getBytes("UTF-8")), "UTF-8");
        } catch (Exception e) {
        }
        return null;
    }
    ...
    public static String byte2hex(byte[] b) {
        ...
        for (int n = 0; n < b.length; n++) {
            String hexString = Integer.toHexString(b[n] & 0XFF).toUpperCase();
            if (hexString.length() == 1) {
                hexBuilder.append("0").append(hexString);
            } else {
